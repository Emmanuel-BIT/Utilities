...
class QueryBuilder
{
    ...
    public function __construct()
    {
    	...
    	/*
    	NB:-$status sera un tableau associatif, d'ou l'utilisation de la boucle foreach pour faciliter les choses
    ...
    public function Connexion($table = '', $columns = array(), $values = array(), $return)
    {
		try
        {
		    ...
		    if($req->rowCount()==1)
		    {
		        $check = $ligne[$return]; //On retourne lid de lutilisateur pour dire que tout c'est bien passé
		    }
		    //Sinon c'est que le mot de passe ou le nom d'utilisateur n'est pas bon
		    else
		    {
		        $check = 0;//Si c'est pas bon on renvoie false
		    }
		    $req->closeCursor();
		    return $check;
		}
                ...
                if (is_object($donnees))
                {
                    while($nuplet = $donnees->fetch())
                    {
                        ...
                        for($i=0; $i < count($headers); $i++)
                        {
                            ...
                            if(substr_count($headerType[$i], 'varchar') > 0 || substr_count($headerType[$i], 'text') > 0 || substr_count($headerType[$i], 'blob') > 0 || substr_count($headerType[$i], 'date') > 0)
                            {
